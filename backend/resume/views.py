...
from .models import Leadership, Description
...
def refresh_pdf(request):
    try:
        print("start running")
        minor_experiences = list(Leadership.objects.values())    
...
def reset_panel(request):
    try:
        ...
        async_to_sync(channel_layer.group_send)(
            "pdf_preview_group", 
            {"type": "send_leadership_panel"} # this must matches the method name in consumers.py
        ) 
        return JsonResponse({"success": True})
    except:
        return JsonResponse({"success": False}, status=400)
...
def add_experience(request):
    if request.method == "POST":
        name = request.POST.get("leadership-name")
        date = request.POST.get("leadership-date")
        location = request.POST.get("leadership-location")
        organisation = request.POST.get("leadership-organisation")
        descriptions = request.POST.getlist("leadership-descriptions[]")  # list of strings

        work_exp = Leadership.objects.create(
            name=name,
            date=date,
            location=location,
...
class ManageView(APIView):
    ...
    def post(self, request):
        try: 
            ...
            if query_type == "leadership":
                ...
                minor_exp = Leadership.objects.create(
                    name=name,
                    date=date,
                    location=location,
                    organisation=organisation
    ...
    def delete(self, request):
        ...
        try:
            query_type = request.query_params.get("type")
            if query_type == "leadership":
                object_delete = Leadership.objects.get(id=object_id)
    ...
    def patch(self, request):
        ...
        try:
            query_type = request.query_params.get("type")
            if query_type == "leadership":
                object_edit = Leadership.objects.get(id=object_id)
